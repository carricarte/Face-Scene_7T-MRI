...
cfg = dict(execution={'use_relative_paths': True,
                      'hash_method': 'content'})
...
for f in os.listdir(input_dir):
    if "INV2" in f and f.endswith(".nii"):
        inv2_file = os.path.join(input_dir, f)
    if "UNI" in f and f.endswith(".nii"):
        uni_file = os.path.join(input_dir, f)
...
def mprageize(inv2_file, uni_file, out_file=None):
    ...
    # tissue1 = ((os.path.join(spm_path, 'tpm/TPM.nii'), 1), 2, (True, False), (False, False))
    # tissue2 = ((os.path.join(spm_path, 'tpm/TPM.nii'), 2), 2, (True, False), (False, False))
    # tissue3 = ((os.path.join(spm_path, 'tpm/TPM.nii'), 3), 2, (True, False), (False, False))
    # tissue4 = ((os.path.join(spm_path, 'tpm/TPM.nii'), 4), 2, (False, False), (False, False))
    # tissue5 = ((os.path.join(spm_path, 'tpm/TPM.nii'), 5), 2, (False, False), (False, False))
    # tissue6 = ((os.path.join(spm_path, 'tpm/TPM.nii'), 6), 2, (False, False), (False, False))
...
def mp2rage_recon_all(inv2_file, uni_file, output_fs_dir=None):
    ...
    if output_fs_dir:
        ...
        fs_dir = output_fs_dir
        sub = 'freesurfer'
    else:
        fs_dir = cwd
        sub = 'freesurfer'

    print(uni_mprageized_file)
    print(fs_dir)
...
mp2rage_recon_all(inv2_file, uni_file, output_dir)
